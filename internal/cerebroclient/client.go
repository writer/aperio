...
// Config contains the minimal hosted Cerebro connection settings Aperio needs.
// The browser-facing Aperio session remains cookie-based; this credential is
...
func (c Config) DefaultRuntime() SourceRuntime {
	return SourceRuntime{
		ID:       envOrValue(c.RuntimeID, DefaultRuntimeID),
		SourceID: envOrValue(c.SourceID, DefaultSourceID),
...
func envDefault(key string, fallback string) string {
	return envOrValue(trimEnv(key), fallback)
}

func envOrValue(value string, fallback string) string {
