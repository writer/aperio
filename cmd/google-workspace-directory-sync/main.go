...
func drainWakeNotifications(ctx context.Context, conn *pgx.Conn, worker *googleworkspacedirectorysync.Sync, notifyDB *sql.DB) {
	...
	for {
		...
		if time.Now().After(deadline) {
			if active.Load() > 0 {
				if !listenerFailed {
					log.Printf("google-workspace-directory-sync: -once wake drain budget elapsed; waiting for %d active sync(s) to finish", active.Load())
				}
				listenerFailed = true
				continue
			}
		...
		if err != nil {
			...
			if active.Load() > 0 {
				log.Printf("google-workspace-directory-sync: -once wake drain stopped listening while %d sync(s) finish: %v", active.Load(), err)
				listenerFailed = true
				continue
			}
